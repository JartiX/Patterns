...
class service_test(unittest.TestCase):
    ...
    def test_check_create_turns_by_nomenclature(self):
        ...
        stop_date = datetime.strptime("2029-12-30", "%Y-%m-%d")
    ...
    def test_check_create_turns_by_nomenclature(self):
        ...
        # Действие
        result = service.create_turns_by_nomenclature_and_storage(start_date, stop_date, nomenclature, storage_ )
    ...
    #
    # Проверить метод  build_debits_by_receipt. Ошибочный сценарий. (обьем превышает оборот)
    #   
    def test_check_build_debits_fail(self):
        # Подготовка
        manager = settings_manager()
        start = start_factory(manager.settings)
        start.create()
        ...
        transactions_data = start.storage.data[ key ]
        transactions_data[0].unit = unit_model.create_gram()
    ...
    #
    # Проверить метод  build_debits_by_receipt. Корректный сценарий. Обьем не превышает оборот
    #   
    def test_check_build_debits_pass(self):
        # Подготовка
        manager = settings_manager()
        start = start_factory(manager.settings)
        start.create()
