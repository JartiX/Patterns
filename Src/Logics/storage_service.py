...
class storage_service:
    ...
    def __process(self, data, key):
        processing = process_factory().create( key )

        data_ = processing().process(data)
    ...
    def create_turns(self, start_period: datetime, stop_period:datetime ) -> list:
        ...
        turns = self.__process(filter.data, key_turn)
        return turns
    ...
    def create_turns_by_nomenclature_and_storage(self, start_period: datetime, stop_period: datetime, nomenclature: nomenclature_model, storage: storage_model) -> list:
        """
            Получить обороты за период по конкретной номенклатуры
        Args:
            start_period (datetime): Начало
    ...
    def create_turns_by_receipt(self, receipt: receipe_model) -> list:
        ...
        # Подобрать процессинг    
        key_turn = process_factory.turn_key()
        turns = self.__process(transactions, key_turn)
    ...
    def build_debits_by_receipt(self, receipt: receipe_model) -> list:
        ...
        for item in receipt.rows():
                for transaction in turns:
                    if item.nomenclature.id == transaction.nomenclature.id:
                        tr1 = transaction.value * transaction.unit.coefficient
                        tr2 = item.size * item.unit.coefficient
                        if tr2 >= tr1:
                            raise operation_exception("Невозможно сформировать транзакцию! Обьем превышает обьем транзакции")
